...
// UPDATED PHARMACY PROFILE INTERFACE WITH PICKUP FIELDS
...
const PharmacyProfilePage = () => {
  const { pharmacyId } = useParams() as { pharmacyId?: string };
  ...
  useEffect(() => {
    ...
    async function fetchPharmacy() {
      try {
        ...
        const res = await fetch(`/api/pharmacies/${pharmacyId}/profile`);
        const data = await res.json();

        if (!res.ok) throw new Error(data?.error || "Pharmacy not found");
    ...
    async function fetchProducts() {
      try {
        ...
        const res = await fetch(
          `/api/pharmacy-products?pharmacyId=${pharmacyId}`,
          { cache: "no-store" }
        );
        ...
        const mapped: Product[] = payload.map((p: any) => ({
          id: p._id,
          name: p.name ?? "Untitled Product",
          price: p.price ?? 0,
          image: p.imageUrl || "/images/product-placeholder.jpg",
  ...
  const handleAddToCart = (product: Product) => {
    ...
    addToCart({
      ...
      vendorId: pharmacyId || "",
      vendorName: pharmacy.pharmacyName || pharmacy.businessName || "",
      vendorBaseLocation: pharmacy.pickupZone || 'Eziobodo',
      vendorRole: 'pharmacy', // 🔥 ADDED: Pharmacy role
      quantity: quantity,
    });
    ...
    return (
      <section className="min-h-screen bg-cream pt-20 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-dark font-medium">Loading pharmacy profile...</p>
        </div>
      </section>
    );
  }
    ...
    return (
      <section className="min-h-screen bg-cream pt-20 pb-12 px-4 md:px-8 flex items-center justify-center">
        <div className="max-w-md mx-auto text-center">
          <h2 className="text-2xl font-bold text-red-600 mt-4">Pharmacy not found</h2>
          <p className="mt-2 text-dark">No pharmacy found for ID: {String(pharmacyId)}</p>
          <button
            onClick={() => router.push("/")}
  ...
  return (
    <section className="min-h-screen bg-cream pt-20 pb-12 px-4 md:px-8">
      ...
      >
        {/* Pharmacy Header Section - UPDATED LIKE CHEF PAGE */}
        <motion.div
        ...
        >
          ...
          >
            {pharmacy.logoUrl ? (
              <img
                ...
                alt={pharmacy.pharmacyName || "Pharmacy"}
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row items-center md:items-end justify-between gap-4 mb-4">
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-olive-2">
                  {pharmacy.pharmacyName || pharmacy.businessName}
                </h1>
                
                {/* UPDATED: Display pickup zone and address exactly like chef/vendor page */}
                <div className="flex flex-col items-center md:items-start gap-1 mt-2">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-dark font-medium">
                      Pickup Area: {pharmacy.pickupZone || "Location not specified"}
                    </p>
                  </div>
                  {pharmacy.pickupAddress && (
                ...
                <p className="text-sm text-mustard mt-1">
                  {pharmacy.category || "Pharmacy & Healthcare"}
                </p>
              </div>
            ...
            <p className="text-dark mt-4 text-lg leading-relaxed">
              {pharmacy.bio || "Trusted pharmacy offering quality medicines and healthcare products."}
            </p>
            
            {/* Pharmacy details with added pickup phone */}
            <div className="flex flex-wrap items-center gap-4 mt-4 text-sm text-dark">
              {pharmacy.minOrder && (
                <span className="flex items-center gap-1">
                  ...
                  Min order: ₦{pharmacy.minOrder.toLocaleString()}
                </span>
              )}
              {/* ADDED: Pickup phone display */}
              <span className="flex items-center gap-1">
                <i className="fas fa-phone text-mustard"></i>
                Pickup Phone: {pharmacy.pickupPhone || "Not provided"}
              </span>
              <span className="flex items-center gap-1">
          ...
          >
            {products.length === 0 ? (
              <div className="col-span-full text-center py-12">
                ...
                <p className="text-dark/70 mt-1">Check back soon for new products!</p>
              </div>
            ) : (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-sm text-dark mb-4">
                      {product.description || "Quality healthcare product from trusted pharmacy."}
                    </p>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg mb-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  disabled
                  title="Place an order first to review this pharmacy"
                >
              ...
              <div className="p-6">
                ...
                <p className="text-dark mb-4">
                  {selectedProduct.description ||
                    "Quality healthcare product from trusted pharmacy."}
                </p>
