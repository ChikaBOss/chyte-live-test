...
export default function VendorsPage() {
  ...
  // load
  useEffect(() => {
    setPending(JSON.parse(localStorage.getItem("pendingVendors") || "[]"));
    setAccounts(JSON.parse(localStorage.getItem("vendorAccounts") || "[]"));
  }, []);

  function savePending(next: Pending[]) {
    localStorage.setItem("pendingVendors", JSON.stringify(next));
    setPending(next);
  }
  
  function saveAccounts(next: Account[]) {
    localStorage.setItem("vendorAccounts", JSON.stringify(next));
    setAccounts(next);
  }
                  ...
                  <tr>
                    <td className="px-6 py-8 text-center text-gray-500" colSpan={6}>
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-sm">Pending vendor requests will appear here</p>
                      </div>
                    </td>
                  </tr>
                )}
