...
export default function PharmacyOverviewPage() {
  ...
  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("pharmacyOrders") || "[]");
    setOrders(saved);
  }, []);
  ...
  const netToPharm   = revenueTotal - adminCut;

  const lowStock = 3; // placeholder metric
  ...
  const last7 = useMemo(()=>{
    ...
    for (let i=6;i>=0;i--){
      ...
      list.push({ label: `${d.getMonth()+1}/${d.getDate()}`, amount });
    }
    return list;
  }, [orders]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold">Pharmacy Overview</h1>
          <p className="text-gray-600">Sales, orders, and inventory highlights.</p>
        </div>
        <div className="flex gap-2">
          <Link href="/pharmacyDashboard/products" className="px-4 py-2 rounded bg-dark text-cream hover:bg-green transition">
            Add Product
          </Link>
          <Link href="/pharmacyDashboard/orders" className="px-4 py-2 rounded border hover:bg-gray-100">
            View Orders
          </Link>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <div className="bg-white rounded shadow p-5">
          ...
          <p className="text-3xl font-bold text-green-600">₦{netToPharm.toLocaleString()}</p>
        </div>
        <div className="bg-white rounded shadow p-5">
          <p className="text-xs uppercase text-gray-500">Open Orders</p>
          <p className="text-3xl font-bold">{openTickets}</p>
        </div>
        <div className="bg-white rounded shadow p-5">
          <p className="text-xs uppercase text-gray-500">Low Stock Items</p>
          ...
          {recent.length === 0 ? (
            ...
            <div key={o.id} className="p-5 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">{o.patient}</p>
              </div>
