...
export default function SearchPage() {
  ...
  const handleAddToCart = (item: SearchResult) => {
    addToCart({
      ...
      description: '', // not available from search result, can be omitted or fetched later
  ...
  const getVendorLink = (item: SearchResult) => {
    switch (item.vendorType) {
      ...
      case 'chef':
        return `/chefs/${item.vendorId}`;
      case 'pharmacy':
        return `/pharmacy/${item.vendorId}`;
      case 'topvendor':
        return `/topVendors/${item.vendorId}`;
      default:
        return '#';
    }
  };
