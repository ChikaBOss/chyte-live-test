...
export default function PharmacyLoginPage() {
  ...
  useEffect(() => {
    const auth = typeof window !== "undefined" ? localStorage.getItem("pharmacyAuth") : null;
    if (auth) router.replace("/pharmacyDashboard");
  }, [router]);
  ...
  function onSubmit(e: FormEvent) {
    ...
    // mock session
    localStorage.setItem("pharmacyAuth", JSON.stringify({ pharmacyId: "p1", email }));
  ...
  return (
    <section className="min-h-[70vh] flex items-center justify-center bg-cream px-6 py-12">
      <div className="w-full max-w-md bg-white rounded-lg shadow p-6">
        ...
        <p className="text-sm text-gray-600 mb-6">Manage products, orders, and earnings.</p>
